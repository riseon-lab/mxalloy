...
class Flux2Transformer(nn.Module):
    ...
    def __call__(
        ...
        if (
            ...
            encoder_hidden_states = self._cached_context
        else:
            projected = self.context_embedder(encoder_hidden_states)
            mx.eval(projected)
            self._cached_context = projected
            self._cached_context_key = encoder_hidden_states
